...
export default class Page extends React.Component<RouteComponentProps<{}>,any> {
  ...
  public dataFetchRemindMe() {
    ...
    fetch(`http://127.0.0.1:8000/doitlaterfi/`, {
        method: 'POST',
        headers: {
  ...
  public dataFetchAddToWatchList(){
    axios.get('http://127.0.0.1:8000/addtowatchlistfi/',{
          headers: {
            'Authorization': `Token ${this.state.TOKEN_NAME}`
          }
        }
  ...
  public dataFetchOnlyWatchList(){
    axios.get('http://127.0.0.1:8000/onlyforwatchlistfi/',{
          headers: {
            'Authorization': `Token ${this.state.TOKEN_NAME}`
          }
        }
  ...
  render() {
    ...
    const decideDirection = () => {
      ...
      if(RemindMe === "Now"){
        return <IonButton onClick={() => {this.dataFetchRemindMe(); this.dataFetchAddToWatchList(); this.dataFetchOnlyWatchList(); this.props.history.push('/page62/Page62');} }  className="cus-color mt-4"  expand="full">Proceed</IonButton>
      }
      else{
    ...
    return (
      ...
      <IonPage className="z-999">
        ...
        <IonContent>
            ...
            <img src="/assets/icon/monitorFI.svg" className="" alt=""></img>
