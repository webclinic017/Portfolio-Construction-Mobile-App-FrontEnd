...
export default class Page extends React.Component<RouteComponentProps<{}>,any> {
  ...
  datafetch(){
    ...
    axios.get('http://127.0.0.1:8000/myholdingdisplayfi/',{
                      headers: {
                  ...
                  .then(response =>{
                    ...
                    this.setState({MYHOLDINGFI : response.data['MyHolding']})
                    this.setState({AMOUNTFI : response.data['Amount']})
                    this.setState({DAILYPROFITFI : response.data['DailyProfit']})
                  })
                  .catch(error => {
                    console.log(error)
                  })
        ...
        <IonContent>
          ...
          <IonRow className="px-70">
            ...
            <IonCol size="6" className="segment2-bg pl-0">
              <button className="ft-12 cus-button4"  onClick={() => { console.log("OK"); this.props.history.push('/page31/Page31'); } } >Watchlist</button>
            </IonCol>
          </IonRow>
          ...
          <p className="text-center f-14">Swipe left to know more</p>
            <IonItem className="cus-input1 mb-12 mt-5">
              ...
              <IonCol size="6" className="text-right">
                <h5 className="main-color">₹{this.state.AMOUNTFI}</h5>
              </IonCol>
            </IonItem>
            <IonItem className="cus-input1 mb-12 mt-5">
              ...
              <IonCol size="6" className="text-right">
                <h5 className="main-color">₹{this.state.DAILYPROFITFI}</h5>
              </IonCol>
            </IonItem>
            <IonList>
              {this.state.MYHOLDINGFI.map((art: any) => { return this.renderMyHoldingFIList(art); })}
            </IonList>
